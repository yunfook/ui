...
function Row({ children, className, style, ref, ...props }: RowProps) {
  ...
  const registerMain = React.useCallback((node: HTMLDivElement, register: boolean) => {
    ...
    if (process.env.NODE_ENV !== "production" && set.size > 1) {
      console.warn(
        "[Row] Multiple <RowItem main> detected in one <Row>. Only one is supported; using the first."
      )
    }
  ...
  return (
    <RowContext.Provider value={{ mainHeight, registerMain }}>
      <div
        ref={ref}
        className={cn(
          "flex w-full min-w-0 flex-row items-start gap-4",
          className
        )}
        style={mergedStyle}
        {...props}
...
function RowItem({
  ...
  const syncHeight = !main && context?.mainHeight != null
  ...
  return (
    <div
      ...
      className={cn(
        ...
        width && "w-[var(--yf-row-width)] flex-none",
        !main && "overflow-auto",
        syncHeight &&
          "h-[var(--yf-row-main-height)] max-h-[var(--yf-row-main-height)]",
        className
      )}
